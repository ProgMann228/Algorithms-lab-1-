...
def write_encoded_to_file(encoded_data, output_file):
    with open(output_file, 'wb') as f:
        for (offset, length, next_byte) in encoded_data:
            f.write(offset.to_bytes(2, byteorder='big'))  # offset как 2 байта
...
if __name__ == "__main__":
    ...
    file3 = "D:/новая папка/PyCharm Community Edition 2024.2.3/sem2Laba1Alg/HAcoded.txt"
...
decoded_image = "D:/новая папка/PyCharm Community Edition 2024.2.3/sem2Laba1Alg/HAdecoded.txt"
...
with open(file_text, 'rb') as f:
    str=f.read()

encoded_data = funk_LZ77(str,buffer_size)
write_encoded_to_file(encoded_data, encoded_image)
encoded_data_from_file = read_encoded_from_file(encoded_image)
decoded_data = iLZ77(encoded_data_from_file)

# Запись декодированных данных в файл
with open(decoded_image, 'wb') as f:
    f.write(decoded_data)

original_size = len(str)
compressed_size = len(encoded_data) *7 # добавляем размер заголовка с индексами
